...
function checkForAndHandleHit() {
	...
	if (checkHit) {
		// Check for hit
		ax= myobjects[5].getByName("BC").getCoordinateTap().getX();
		ay= myobjects[5].getByName("BC").getCoordinateTap().getY();
		var ar = myobjects[5].getByName("BC").getCoordinateTap().getR() * sioux.sx * sioux.rsx; // radius
		//console.log("Check: "+myobjects[5].getByName("BC").getCoordinateTap().getName()+" "+ax+" "+ay+" "+ar);
		...
		for ( var i = 0; i < 5; i++) {
			...
			var r2=  (tr + ar)
					* (tr + ar);
			 //console.log(i+" "+scale+" "+tx+" "+ty+" "+tr+" "+((tx - ax) * (tx - ax) + (ty - ay) * (ty - ay))+" <? "+r2);
			...
			if ((tx - ax) * (tx - ax) + (ty - ay) * (ty - ay) <= r2) {
				if (services.getView().pa != undefined) { // arrow is flying
					...
					tatanka.hits++;
					var phi = (tatanka.rot + tatanka.rrot);
					phi = Math.round(phi);
					while (phi < 0)
						phi += 360;
					phi %= 360;
					...
					//console.log("t.x: "+tatanka.x);
					ax= myobjects[5].x-tatanka.x-tatanka.rx;
					ay= myobjects[5].y-tatanka.y-tatanka.ry;
				
					dx=  (ax*cosphi - ay * sinphi);
					dy=  (ay*cosphi + ax * sinphi);
					narr.getByName("stone").translateRoot(20000, 20000, 0);	
					narr.translate(-narr.x+dx, -narr.y+dy, 0);
				
				//	alert("t.a11: "+tatanka.coords.a11+" t.a21: "+tatanka.coords.a21+" t.a12: "+tatanka.coords.a12+" t.a12: "+tatanka.coords.a22);
					...
					myobjects[5].setVisibility(false);
					myobjects[5].reset();
					myobjects[5].getByName("BC").getCoordinateTap().reset();
					checkHit = false;
					hit = false;
					return;
				}
...
function initTatanka(c) {
	...
	myobjects[5]= new Arrow();
	myobjects[5].scaleRoot(scale, scale);
	myobjects[5].setVisibility(false);
	myobjects[5].getCoordinateTap().setName("FLYING ARROW");
	goodie = new Goodie();
	goodie.create(1);
	myobjects[6] = goodie;
	myobjects[7] = goodie.bc;
	// myobjects[7] = rope;
...
function shootStep4() {
	...
	var l = Math.sqrt(canvaswidth * canvaswidth + canvasheight * canvasheight)
			/ scale;
	myobjects[5].translate(-myobjects[5].x+arr.coords.x,-myobjects[5].y+arr.coords.y);
	var arot= sioux.rot+ rest.rot+ body.rot+ la.rot+ bow.rot+ arr.rot+sioux.rrot+ rest.rrot+ body.rrot+ la.rrot+ bow.rrot+ arr.rrot;
	myobjects[5].rotate(-myobjects[5].rot+arot);
	myobjects[5].setVisibility(true);
	
	var phi = (myobjects[5].rot + myobjects[5].rrot);
	phi = Math.round(phi);
	while (phi < 0)
		phi += 360;
	phi %= 360;
	...
	services.getView().pa = new PartAnimation();
	services.getView().pa.init(myobjects[5], -sinbeta*l, -cosbeta*l, 0, 1.0, 1.0, 1500, false);
	services.getView().pa.start();
	checkHit = true;
